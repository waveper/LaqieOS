    ...
    */
    ...
    if (argc == 0) {
      ...
      for (unsigned int i = 0;
           EXEC_PREFIX[i] != '\0' && pos < sizeof(execbuffer) - 1; ++i) {
        execbuffer[pos++] = EXEC_PREFIX[i];
      }
      for (unsigned int i = 0;
           argv[0][i] != '\0' && pos < sizeof(execbuffer) - 1; ++i) {
        execbuffer[pos++] = argv[0][i];
      }
      for (unsigned int i = 0;
           EXEC_SUFFIX[i] != '\0' && pos < sizeof(execbuffer) - 1; ++i) {
        execbuffer[pos++] = EXEC_SUFFIX[i];
      }
      execbuffer[pos] = '\0';
      target_pid = exec(execbuffer);
      if (target_pid == -1) {
